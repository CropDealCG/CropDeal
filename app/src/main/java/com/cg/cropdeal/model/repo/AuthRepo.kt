                        ...
                        FirebaseDatabase.getInstance().reference.child(Constants.USERS)
                            ...
                            .addListenerForSingleValueEvent(object : ValueEventListener{
                                override fun onDataChange(snapshot: DataSnapshot) {
                                    application!!.getSharedPreferences(Constants.LOGINPREF, Context.MODE_PRIVATE)
                                        .edit {
                                            putString(Constants.USERTYPE,snapshot.child(Constants.TYPE).value.toString())
                                            apply()
                                        }
                                        signInFailed!!.postValue(false)
                                        userLiveData!!.postValue(firebaseAuth!!.currentUser)
...
//        dialog.setMessage("Are you a?")
        ...
        customBinding.typeInfo.text = context.getString(R.string.selectUserTypeDialogText)
        customBinding.typeRadioGroup.setOnCheckedChangeListener { _, i ->
            when(i){
                customBinding.dealerRadio.id ->{selectedUserType!!.postValue(Constants.DEALER)}
                customBinding.farmerRadio.id -> {selectedUserType!!.postValue(Constants.FARMER)}
            }
        }
