...
class AdminFarmerVM(application: Application) : AndroidViewModel(application) {
    ...
    private fun populateList() {
        firebaseDatabase!!.reference.child(Constants.USERS).orderByChild(Constants.TYPE).equalTo(Constants.FARMER)
...
//                                Log.d("Observables","${task.result?.user?.uid!!},${task.result?.additionalUserInfo?.username},${task.result?.user?.email!!}")
                        ...
                        val user = Users(name,email,Constants.FARMER,"false","","", Payment(),true,0.0,0,"")
                ...
                Toast.makeText(getApplication<Application>().applicationContext!!,"Registration Failure: ${task.exception?.message}",Toast.LENGTH_LONG).show()
            }
        }
    }
